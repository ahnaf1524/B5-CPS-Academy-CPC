...
ll nCr(int n, int r) {
  return (r < 0 || r > n) ? 0 : fac[n] * ifac[r] % MOD * ifac[n - r] % MOD;
}
ll nPr(int n, int r) {
  return (r < 0 || r > n) ? 0 : fac[n] * ifac[n - r] % MOD;
}

// Frequency utilities
template <typename T>
map<T, int> count_freq(const vector<T> &v) {
  map<T, int> f;
  for (auto &i : v) f[i]++;
  return f;
}
map<char, int> count_chars(const str &s) {
  return count_freq(vector<char>(s.begin(), s.end()));
}
inline ll count_specific_char(const str &s, char c) { return count(all(s), c); }
inline int count_digit(ll n, int d) {
  int c = 0;
  while (n) {
    c += (n % 10 == d);
    n /= 10;
...
inline int digit_sum(ll n) {
  int s = 0;
  while (n) s += n % 10, n /= 10;
  return s;
}
inline ll remove_duplicate_digits(ll n) {
  str s = to_string(n);
  unordered_set<char> u;
  str r;
  for (char c : s)
    if (!u.count(c)) r += c, u.insert(c);
  return stoll(r);
}
inline bool all_chars_same(const string &s) {
  return s.empty() || s.find_first_not_of(s[0]) == string::npos;
}
template <typename T>
inline bool all_elements_same(const vector<T> &v) {
  return v.empty() || all_of(next(v.begin()), v.end(),
                             [&](const T &x) { return x == v[0]; });
}
inline bool all_digits_same(ll n) {
  for (int d = n % 10; n; n /= 10)
...
vi string_to_digit_vector(const str &s) {
  vi d;
  for (char c : s)
    if (isdigit(c)) d.pb(c - '0');
  return d;
}
vi prefix_sum(const vi &v) {
  vi p(sz(v) + 1);
  for (int i = 0; i < sz(v); ++i) p[i + 1] = p[i] + v[i];
  return p;
}
pair<int, int> count_even_odd(const vi &v) {
  int e = 0, o = 0;
  for (int x : v) (x % 2 == 0 ? e : o)++;
  return {e, o};
}
template <typename T>
